...
import { VerifiedCommentItem as VerifiedCommentItemType } from '@/shared/config/api/api.schemas';
import { useScammersGetCommentList } from '@/shared/config/api/scammers/scammers';
import { Col, Row } from '@/shared/ui/boxes';
import { Button, Typography } from '@mui/material';
import { useEffect, useState } from 'react';
import { useVerifiedGetCommentList } from '@/shared/config/api/verified/verified';
import { VerifiedCommentItem } from './verified.comment.item';

export const VerifiedProfileComments = ({ id }: { id: number }) => {
    const { data } = useVerifiedGetCommentList(id);
    const [commentsCount, setCommentsCount] = useState(5);
    const [comments, setComments] = useState<VerifiedCommentItemType[]>();
    ...
    return (
        <Col gap={2}>
            ...
            <Col bgcolor={'#ECF2FF'} borderRadius={'9px'} py={2.25} px={1.75} gap={2}>
                {comments.map((c, i) => (
                    <VerifiedCommentItem comment={c} key={i} />
                ))}
