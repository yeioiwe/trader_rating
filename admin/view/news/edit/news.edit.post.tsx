'use client';
import { useNewsEdit, useNewsGetOne } from '@/config/api/news/news';
...
export const NewsEditPost = ({ id }: { id: number }) => {
    const { register, getValues, setValue } = useForm();
    const { data: newsPost } = useNewsGetOne(id);
    const { mutate } = useNewsEdit();
    ...
    useEffect(() => {
        if (newsPost) {
            ...
            setImg(newsPost.avatar);
            setDate(newsPost.date);
            setValue('url', newsPost.url);
            setValue('likes', newsPost.likes);
            setValue('title', newsPost.title);
            setValue('views', newsPost.views);
        }
    }, [newsPost]);
    ...
    if (newsPost === undefined) return null;
    return (
        <Col gap={4} width={'100%'}>
