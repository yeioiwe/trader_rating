'use client';
import { ScammerProfileAboutVisible } from '@/config/api/api.schemas';
import { useScammersEditAbout, useScammersGetAbout } from '@/config/api/scammers/scammers';
...
export const ScammersEditAbout = ({ id }: { id: number }) => {
    const { data: projectAbout } = useScammersGetAbout(id);
    const { register, getValues, setValue } = useForm();
    const { mutate } = useScammersEditAbout();

    const [about, setAbout] = useState('');
    const [visible, setVisible] = useState<ScammerProfileAboutVisible>(ScammerProfileAboutVisible.VISIBLE);
